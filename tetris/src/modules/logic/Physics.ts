...
export class Physics {
  ...
  public checkCollision(
    ...
    pivotBoardY: number,
    shape: number[][],
    pieceTypeKey?: keyof typeof TETROMINOES,
  ): boolean {
    const typeKey =
      pieceTypeKey || (this.gameState.currentTetromino?.typeKey as keyof typeof TETROMINOES);
    ...
    for (let r_shape = 0; r_shape < shape.length; r_shape++) {
      for (let c_shape = 0; c_shape < shape[r_shape].length; c_shape++) {
        if (shape[r_shape][c_shape]) {
          ...
          if (
            boardY >= 0 &&
            this.gameState.board[boardY] &&
            this.gameState.board[boardY][boardX] !== null
          ) {
  ...
  public rotate(direction: 'clockwise' | 'counter-clockwise' | '180'): {
    success: boolean;
    landed: boolean;
  } {
    ...
    const kicks: number[][] = (typeKey === 'I'
      ? KICK_DATA_I[kickTableKey as keyof typeof KICK_DATA_I]
      : KICK_DATA_JLSTZ[kickTableKey as keyof typeof KICK_DATA_JLSTZ]) || [[0, 0]];
    ...
    for (let i = 0; i < kicks.length; i++) {
      ...
      if (!this.checkCollision(newX, newY, nextShape)) {
        console.debug(
          `[Physics] Rotation Success - Kick #${i} (${kick[0]}, ${kick[1]}) -> New Pos: (${newX}, ${newY}), Rotation: ${nextRotationState}`,
        );
    ...
    console.debug(`[Physics] Rotation Failed - No valid kicks for ${typeKey}`);
    return { success: false, landed: this.gameState.isPieceLanded };
  }

  private handlePostSuccessfulMoveRotation(): boolean {
    const landedAfterMove = this.checkCollision(
      this.gameState.currentTetromino!.x,
      this.gameState.currentTetromino!.y + 1,
      this.gameState.currentTetromino!.shape,
    );
  ...
  public moveBlockLeft(): { success: boolean; landed: boolean } {
    ...
    if (
      !this.checkCollision(
        this.gameState.currentTetromino.x - 1,
        this.gameState.currentTetromino.y,
        this.gameState.currentTetromino.shape,
      )
    ) {
  ...
  public performHardDrop(): {
    clearedLines: number;
    gameOver: boolean;
    displayInfo?: { clearType: string; b2bCount: number; comboCount: number };
  } {
    ...
    return {
      clearedLines: lockResult.clearedLines,
      gameOver: false,
      displayInfo: lockResult.displayInfo,
    };
  }

  public lockTetromino(): {
    clearedLines: number;
    gameOver: boolean;
    displayInfo?: { clearType: string; b2bCount: number; comboCount: number };
  } {
    ...
    for (let r_shape = 0; r_shape < shape.length; r_shape++) {
      for (let c_shape = 0; c_shape < shape[r_shape].length; c_shape++) {
        if (shape[r_shape][c_shape]) {
          ...
          if (
            boardY >= 0 &&
            boardY < LOGICAL_BOARD_HEIGHT_BLOCKS &&
            boardX >= 0 &&
            boardX < BOARD_WIDTH_BLOCKS
          ) {
  ...
  private checkForCompletedLinesWithDisplay(): {
    clearedLines: number;
    displayInfo?: { clearType: string; b2bCount: number; comboCount: number };
  } {
  ...
  private updateScoringAndGetDisplayInfo(
    linesCleared: number,
    spinInfo?: { isSpin: boolean; pieceType: string },
    _shouldCheckForSpin?: boolean,
  ): {
    clearedLines: number;
    displayInfo: { clearType: string; b2bCount: number; comboCount: number };
  } {
    ...
    const { clearType, isDifficultClear, baseScore } = this.getClearTypeAndScore(
      linesCleared,
      isPerfectClear,
      spinInfo,
      _shouldCheckForSpin,
    ...
    if (
      this.gameState.backToBackActive &&
      isDifficultClear &&
      this.gameState.backToBackCount >= 2
    ) {
  ...
  private getClearTypeAndScore(
    linesCleared: number,
    isPerfectClear: boolean,
    spinInfo?: { isSpin: boolean; pieceType: string },
    _shouldCheckForSpin?: boolean,
  ): { clearType: string; isDifficultClear: boolean; baseScore: number } {
    let clearType = '';
  ...
  private checkSpinCondition(piece: {
    x: number;
    y: number;
    typeKey: string;
    rotation: number;
    ...
    for (const corner of corners) {
      const isOutOfBounds =
        corner.x < 0 || corner.x >= 10 || corner.y < 0 || corner.y >= LOGICAL_BOARD_HEIGHT_BLOCKS;
      const hasBlock =
        !isOutOfBounds &&
        corner.y < this.gameState.board.length &&
        this.gameState.board[corner.y][corner.x] !== null;

      if (isOutOfBounds || hasBlock) {
        filledCorners++;
        cornerDetails.push(`(${corner.x}, ${corner.y}): FILLED${isOutOfBounds ? ' (OOB)' : ''}`);
      } else {
        cornerDetails.push(`(${corner.x}, ${corner.y}): EMPTY`);
      }
    }

    const isSpin = filledCorners >= 3;
    console.debug(
      `[Physics] Spin Check: Piece ${piece.typeKey}, Corners Filled: ${filledCorners}/4 -> ${isSpin ? 'SPIN DETECTED' : 'NO SPIN'}`,
    );
    console.debug(`[Physics] Corner Details: ${cornerDetails.join(' | ')}`);
    return isSpin;
  }
  ...
  public spawnNewTetromino(): { success: boolean; landed: boolean; gameOver: boolean } {
    ...
    const landed = this.checkCollision(
      this.gameState.currentTetromino.x,
      this.gameState.currentTetromino.y + 1,
      this.gameState.currentTetromino.shape,
    );
  ...
  public performHold(): { success: boolean; gameOver: boolean; landed: boolean } {
    if (!this.gameState.canHold) {
      return {
        ...
        landed: this.gameState.isPieceLanded,
      };
    }
  ...
  private spawnHeld(typeKey: keyof typeof TETROMINOES): {
    success: boolean;
    gameOver: boolean;
    landed: boolean;
  } {
